...
import { Clock3, Eye, GitCompare, History, RotateCcw, Save, X } from 'lucide-react'
...
export default function VersionHistoryPanel({ sheetId, current, previewingId, onPreview, onCreate, onRestore, onClose }: Props) {
  ...
  const versions = useMemo(() => diagramVersionService.list(sheetId), [sheetId, revision])
  const selected = versions.find(version => version.id === selectedId) || versions[0] || null
  const diff = selected ? diagramVersionService.diff(selected.snapshot, current, includePosition) : null
  ...
  return <>
    <aside className="flex w-[390px] shrink-0 flex-col border-l border-admin-outline bg-white shadow-[-12px_0_35px_rgba(15,23,42,.06)]">
      <header className="flex h-14 items-center gap-3 border-b border-admin-outline px-4"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-uml-blue/10 text-uml-blue"><History size={17}/></div><div className="min-w-0 flex-1"><h2 className="text-sm font-bold">Version history</h2><p className="text-[9px] font-bold uppercase tracking-widest text-admin-secondary">Stored on this device</p></div><button onClick={onClose} className="rounded-md p-1.5 text-admin-secondary hover:bg-admin-bg"><X size={17}/></button></header>
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        {!versions.length ? <div className="px-8 py-16 text-center"><History size={28} className="mx-auto text-admin-outline"/><p className="mt-3 text-sm font-bold">No saved versions</p><p className="mt-1 text-xs leading-5 text-admin-secondary">Create a checkpoint before a major edit, import, or AI change.</p></div> : <>
          ...
          {selected && <div className="p-3"><div className="mb-3 flex gap-1 rounded-lg border border-admin-outline bg-admin-bg p-1"><button onClick={() => setTab('changes')} className={`flex-1 rounded-md px-2 py-1.5 text-[10px] font-bold ${tab === 'changes' ? 'bg-white text-uml-blue shadow-sm' : 'text-admin-secondary'}`}>Changes ({total})</button><button onClick={() => setTab('side')} className={`flex-1 rounded-md px-2 py-1.5 text-[10px] font-bold ${tab === 'side' ? 'bg-white text-uml-blue shadow-sm' : 'text-admin-secondary'}`}>Side by side</button></div>
    ...
    {createOpen && <SaveVersionDialog onClose={() => setCreateOpen(false)} onSave={(name, note) => { onCreate(name, note); setCreateOpen(false); setRevision(value => value + 1) }}/>} 
    {restoreVersion && <RestoreDialog version={restoreVersion} diff={diagramVersionService.diff(restoreVersion.snapshot, current)} onClose={() => setRestoreVersion(null)} onRestore={() => { onRestore(restoreVersion); setRestoreVersion(null); setRevision(value => value + 1) }}/>} 
    {fullCompare && selected && <FullCanvasCompare version={selected} current={current} includePosition={includePosition} onClose={() => setFullCompare(false)} onRestore={() => { setFullCompare(false); setRestoreVersion(selected) }}/>} 
  </>
}
...
function FullCanvasCompare({ version, current, includePosition, onClose, onRestore }: { version: DiagramVersion; current: DiagramSnapshot; includePosition: boolean; onClose: () => void; onRestore: () => void }) {

    ...
    <div className="flex items-center justify-between gap-4 py-1.5">
      ...
      <span className="flex shrink-0 items-center gap-1">
        {keys.map((k, i) => (
          <kbd
            key={i}
            className="rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 font-mono text-[11px] text-zinc-600 shadow-[0_1px_0_rgba(0,0,0,0.04)]"
          >
            {k}
          </kbd>
        ))}
...
function Section({
  ...
  return (
    <div>
      <p className="mb-1 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-zinc-400/80">
        {title}
      </p>
...
export function HelpOverlay({ onClose }: { onClose: () => void }) {
  return (
    ...
    >
      <div
        className="animate-pop w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.25)]"
        ...
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-3.5">
          <div>
            ...
            <p className="text-[12px] text-zinc-400">
              Move faster — everything is keyboard-driven.
            </p>
          </div>
          <button
            onClick={onClose}

...
export function ContextMenu({
  ...
  // Clamp position so the menu never overflows the window.
  const W = 212;
  ...
  return (
    <div
      ref={ref}
      className="animate-pop fixed z-50 min-w-[200px] overflow-hidden rounded-xl border border-zinc-200 bg-white py-1 shadow-[0_8px_30px_rgba(0,0,0,0.16)]"
      style={{ left: px, top: py }}
      onContextMenu={(e) => e.preventDefault()}
    >
      {items.map((item, i) =>
        item.divider ? (
          <div key={i} className="my-1 h-px bg-zinc-200" />
        ) : (
          <button
            key={i}

...
function FormulaCard({ price, discount }: { price: number; discount: number }) {
  ...
  return <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
    <div className="mb-2 flex items-center gap-1.5"><Calculator className="h-3.5 w-3.5 text-slate-500" /><span className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Công thức tính</span></div>
    <p className="mb-2.5 rounded-lg bg-white px-2.5 py-1.5 text-center font-mono text-[12px] text-slate-600 ring-1 ring-inset ring-slate-200">Giá năm = Giá tháng × 12 × (1 − <span className="text-slate-900">{discount}%</span>)</p>
    ...
    <div className="mt-2.5 grid grid-cols-3 gap-2 border-t border-slate-200 pt-2.5">
      ...
      <div className="rounded-lg bg-white p-2 text-center ring-1 ring-inset ring-slate-200"><p className="text-[13.5px] font-bold text-slate-900">{fmtVnd(c.perMonth)}đ</p><p className="text-[10px] text-slate-400">Quy đổi/tháng</p></div>
...
function PlanCard({ plan, billing, onEdit, onDelete }: { plan: SubscriptionPlan; billing: Billing; onEdit: () => void; onDelete: () => void }) {
  ...
  return <Card className="flex flex-col overflow-hidden hover:border-indigo-200">
    ...
    <div className="flex flex-1 flex-col p-5">
      <div className="flex items-start justify-between gap-2">
        ...
        <Badge tone={statusLabel[plan.status].tone}>{statusLabel[plan.status].label}</Badge>
      </div>
...
function PlanRow({ plan, billing, onEdit, onDelete }: { plan: SubscriptionPlan; billing: Billing; onEdit: () => void; onDelete: () => void }) {
  const price = computePrice(plan, billing);
...
function LimitField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  ...
  return <div>
    ...
    <input type="number" disabled={unlim} value={unlim ? "" : value} onChange={(e) => onChange(Math.max(0, parseInt(e.target.value) || 0))} placeholder={unlim ? "∞" : "0"}
...
function PlanEditor({ draft, setDraft }: { draft: Draft; setDraft: (d: Draft) => void }) {
  ...
  return <div className="space-y-5">
    <div>
      <p className="mb-3 text-[12px] font-bold uppercase tracking-wide text-slate-400">Thông tin chung</p>
      ...
      <label className="mt-3 block"><span className="mb-1 block text-[12.5px] font-medium text-slate-700">Mô tả</span><textarea value={draft.description} onChange={(e) => set({ description: e.target.value })} rows={2} placeholder="Mô tả ngắn về gói…" className={cn(inputCls, "resize-none")} /></label>
    </div>
    <div>
      <p className="mb-3 text-[12px] font-bold uppercase tracking-wide text-slate-400">Giá & chu kỳ thanh toán</p>
      <label className="mb-3 flex cursor-pointer items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      {!draft.contactOnly && <label className="block"><span className="mb-1 flex items-center gap-2 text-[12.5px] font-medium text-slate-700"><CalendarDays className="h-3.5 w-3.5 text-slate-400" /> Giá hàng tháng (VNĐ) <span className="font-normal text-slate-400">— đặt 0 cho gói miễn phí</span></span><input type="number" value={draft.price} onChange={(e) => set({ price: Math.max(0, parseInt(e.target.value) || 0) })} className={inputCls} /></label>}
      {!draft.contactOnly && draft.price > 0 && <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 transition">
        <button type="button" onClick={() => set({ yearlyBilling: !draft.yearlyBilling })} className={cn("flex w-full items-center justify-between gap-3 p-3 text-left transition", draft.yearlyBilling ? "bg-indigo-50" : "bg-white hover:bg-slate-50")}>
          <span className="flex items-center gap-2.5">
            <span className={cn("flex h-8 w-8 items-center justify-center rounded-xl transition", draft.yearlyBilling ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500")}><CalendarDays className="h-4 w-4" /></span>
            <span><span className="block text-[13px] font-medium text-slate-900">Hỗ trợ thanh toán theo năm</span><span className="block text-[11.5px] text-slate-400">{draft.yearlyBilling ? "Đang bật — khách hàng có thể trả 1 lần cả năm để được giảm giá." : "Mặc định chỉ thanh toán theo tháng."}</span></span>
          </span>
          <Switch on={draft.yearlyBilling} onClick={() => set({ yearlyBilling: !draft.yearlyBilling })} />
        </button>
        {draft.yearlyBilling && <div className="animate-fade-in space-y-3 border-t border-slate-100 bg-slate-50 p-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {!discountLevels.includes(draft.yearlyDiscount) && <label className="block"><span className="mb-1 block text-[12.5px] font-medium text-slate-700">% giảm (0 – 50)</span><input type="number" min={0} max={50} value={draft.yearlyDiscount} onChange={(e) => set({ yearlyDiscount: Math.min(50, Math.max(0, parseInt(e.target.value) || 0)) })} className={inputCls} /></label>}
          </div>
          <FormulaCard price={draft.price} discount={draft.yearlyDiscount} />
        </div>}
      </div>}
      <label className="mt-3 flex cursor-pointer items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3">
        <span className="flex items-center gap-2 text-[13px] font-medium text-slate-700"><Crown className="h-4 w-4 text-slate-500" /> Đánh dấu là gói phổ biến</span>
        <Switch on={draft.popular} onClick={() => set({ popular: !draft.popular })} />
      </label>
    </div>
    <div>
      <p className="mb-3 text-[12px] font-bold uppercase tracking-wide text-slate-400">Giới hạn sử dụng</p>
      <div className="grid gap-3 sm:grid-cols-2">
    ...
    <div>
      <div className="mb-3 flex items-center justify-between"><p className="text-[12px] font-bold uppercase tracking-wide text-slate-400">Tính năng đi kèm</p><span className="text-[11px] text-slate-400">{Object.values(draft.features).filter(Boolean).length}/{featureCatalog.length + draft.customFeatures.length} bật</span></div>
    ...
    <div>
      <p className="mb-2 text-[12px] font-bold uppercase tracking-wide text-slate-400">Màu nhận diện</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-5">
        ...
        {plans.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-4">
            {plans.map((p) => <PlanCard key={p.id} billing={billing} plan={p} onEdit={() => setDraft(toDraft(p))} onDelete={() => setConfirmDel(p)} />)}
          ...
          <div className="space-y-2">
            ...
            {plans.map((p) => <PlanRow key={p.id} billing={billing} plan={p} onEdit={() => setDraft(toDraft(p))} onDelete={() => setConfirmDel(p)} />)}
      ...
      <Modal open={!!draft} onClose={() => setDraft(null)} title={draft?.mode === "edit" ? "Chỉnh sửa gói" : "Tạo gói subscription"} desc={draft?.mode === "edit" ? draft?.name : "Cấu hình giá, giới hạn và tính năng cho gói"} size="xl"
        footer={<><button onClick={() => setDraft(null)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-[13px] font-medium text-slate-700 transition hover:bg-slate-50"><X className="h-4 w-4" /> Huỷ</button><button onClick={save} className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3.5 py-2 text-[13px] font-medium text-white transition hover:bg-slate-700"><Check className="h-4 w-4" /> {draft?.mode === "edit" ? "Lưu thay đổi" : "Tạo gói"}</button></>}>

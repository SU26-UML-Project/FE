...
export function ImportModal({ onClose, onImport }: ImportModalProps) {
  ...
  const handleImport = () => {
    ...
    try {
      ...
      if (format === "mermaid") {
        result = parseMermaid(code);
      } else if (format === "plantuml") {
        result = parsePlantUml(code);
      } else {
        // Auto-detect: try to determine if it's Mermaid or PlantUML
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-admin-outline/30 bg-white shadow-2xl animate-pop">
          ...
          <div className="flex items-center justify-between border-b border-admin-outline/10 px-6 py-4 bg-admin-bg/30">
            <h2 className="text-[16px] font-bold text-admin-on-surface">
              Import Mermaid / PlantUML Code
            </h2>
            <button
                onClick={onClose}
                className="rounded-lg p-1 text-admin-secondary/50 hover:bg-admin-bg hover:text-admin-on-surface transition-colors"
            >
              <svg
                  width="20"
                  height="20"
          ...
          <div className="p-6">
            <p className="mb-4 text-[13px] text-admin-secondary">
              Paste your <strong>Mermaid</strong> or{" "}
              <strong>PlantUML</strong> code below (Class, Use Case, Activity,
              Sequence, State, Flowchart, or Component diagrams).
            </p>

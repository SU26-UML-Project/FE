...
const QuestionBox: React.FC<QuestionBoxProps> = ({ questions, onSubmit, isSubmitting }) => {
  ...
  const handleOptionClick = (option: string) => {
    if (currentQuestion.type === 'single_select') {
      ...
      setAnswers(prev => {
        ...
        if (isSelected) {
          return { ...prev, [currentStep]: currentAnswers.filter(o => o !== option) };
        } else {
  ...
  const isOtherSelected = selectedOptions.includes('Khác');
  const isCurrentStepValid = 
    selectedOptions.length > 0 && 
    (!isOtherSelected || otherText.trim() !== '');

  const isAllValid = questions.every((q, idx) => {
    const qAnswers = answers[idx] || [];
    const qOtherText = otherTexts[idx] || '';
    const qOtherSelected = qAnswers.includes('Khác');
  ...
  return (
    <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-4 my-3 space-y-4 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            {questions.map((_, idx) => (
              <div 
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all ${
                  idx === currentStep ? 'bg-uml-blue w-4' : idx < currentStep ? 'bg-uml-blue/40' : 'bg-gray-200'
                }`} 
              />
            ))}
          </div>
      ...
      {/* Question Content */}
      <div className="space-y-3 animate-in fade-in slide-in-from-right-2 duration-200" key={currentStep}>
            ...
            return (
              ...
              >
                {currentQuestion.type === 'single_select' ? (
                  ...
                  isSelected ? <CheckSquare size={14} /> : <Square size={14} className="opacity-30" />
                )}
                {option}
              </button>
            );
        ...
        {isOtherSelected && (
          <div className="animate-in zoom-in-95 duration-200">
            <textarea
              value={otherText}
              onChange={(e) => setOtherTexts(prev => ({ ...prev, [currentStep]: e.target.value }))}

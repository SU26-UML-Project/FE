...
/**
 * Mở rộng kích thước package/boundary/swimlane để bao hết các node con.
 *
 * <p>Trước đây ELK nhận package với size cứng (480x130 cho swimlane, 400x300 cho
 * package từ aiToCanvas.ts) → layout các class có nhiều attributes/methods dồn
 * cục trong kích thước đó, tràn ra ngoài → user thấy "class bị gom lại 1 chỗ".
 *
 * <p>Cách D (kết hợp A+B):
 * <ul>
 *   <li>Function này chạy trong {@link importCanvas} <strong>trước khi gọi
 *       layoutElements</strong>, đảm bảo ELK nhận đúng kích thước tối thiểu.</li>
 *   <li>Tính tổng width/height của các node con + padding 40px mỗi cạnh.</li>
 *   <li>Chỉ mở rộng khi container <em>thực sự</em> có con. Package rỗng giữ
 *       kích thước tối thiểu từ {@link swimlaneMinSize}.</li>
 *   <li>Bỏ qua nếu container đã có kích thước ≥ tổng con (vd: user đã resize tay).</li>
 * </ul>
 */
function expandContainerSizes(nodes: FlowNode[]): FlowNode[] {
    const CONTAINER_TYPES = new Set(["package", "boundary", "swimlane"]);
    const PAD = 40; // mỗi cạnh (top/right/bottom/left)

    // Bước 1: nhóm con theo parentId
    ...
    // Bước 2: với mỗi container, tính tổng kích thước con
    // Lưu ý: tại thời điểm này `position` của con là (0, 0) (do aiToCanvas đặt),
    // nên không thể tính bounding box theo position. Thay vào đó dùng width/height
    // của mỗi con (đã được classMinSize tính từ attributes/methods).
    return nodes.map((node) => {
        if (!CONTAINER_TYPES.has(node.type)) return node;

        const children = childrenByParent.get(node.id);
        if (!children || children.length === 0) {
            // Container rỗng → giữ kích thước hiện tại (đã set bởi aiToCanvas / swimlaneMinSize)
            return node;
        }

        // Tính tổng width/height của các con
        // (dùng width/height cứng vì measured chưa có tại thời điểm này)
        const totalChildArea = children.reduce(
            (sum, c) => sum + (c.width ?? 150) * (c.height ?? 60),
            0
        );
        // Ước lượng kích thước tối thiểu cần thiết
        // Lấy max width của 1 hàng + max height xếp chồng, nhưng đơn giản hóa:
        // tổng diện tích con + padding bao quanh = diện tích tối thiểu của container.
        // Để tỉ lệ đẹp, lấy sqrt(area) cho cả 2 chiều rồi + padding.
        const minSide = Math.ceil(Math.sqrt(totalChildArea)) + PAD * 2;
        const minW = Math.max(node.width ?? 0, minSide);
        const minH = Math.max(node.height ?? 0, minSide);

        return {
            ...node,
            width: minW,
            height: minH,
            // Đảm bảo style cũng cập nhật (nếu có) để React Flow render đúng kích thước
            style: { ...(node.style as object), width: minW, height: minH },
        };
    });
}
...
export function Editor() {
    ...
    const onConnect = useCallback(
        (conn: Connection) => {
            ...
            // [FIX] Chuẩn hóa handle id: React Flow cho phép user kéo từ cả source và
            // target handle. Nếu user kéo từ target handle (id kết thúc bằng `-t`),
            // `conn.sourceHandle` có thể là target id (vd "t-75-t") → React Flow báo
            // warning và không render edge. Bỏ suffix `-t` để thành source id hợp lệ.
            // Tương tự cho targetHandle.
    ...
    const importCanvas = useCallback(
        async (
            ...
            // Bước chèn mới (Cách D - phần B): mở rộng kích thước package/boundary
            // theo tổng kích thước con trước khi ELK layout. Nếu không có bước này,
            // class có nhiều attributes/methods bị dồn cục trong package size cứng.
            inNodes = expandContainerSizes(inNodes);

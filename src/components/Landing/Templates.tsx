...
const templateData = [
  { id: 1, src: classdiagram },
  { id: 2, src: usecase },
  { id: 3, src: activitydiagram },
  { id: 4, src: sequencediagram },
  { id: 5, src: statemachine },
  { id: 6, src: componentdiagram },
]
...
const Templates = () => {
  const t = useT()
  const alts = t.list('templates.alts')
  ...
  return (
    <section id="popular-templates" className="py-20 px-4">
      ...
      >
        <h2 className="text-4xl font-priego-extrabold font-bold text-left mb-12 ml-4 md:ml-0 uppercase tracking-tight text-black">{t('templates.heading')}</h2>
        ...
        >
          ...
          <button
            onClick={prev}
            aria-label={t('templates.prevAria')}
          ...
          <button
            onClick={next}
            aria-label={t('templates.nextAria')}
        ...
        <nav aria-label={t('templates.navAria')} className="flex justify-center items-center space-x-4 mt-12" data-purpose="slider-pagination">
          {Array.from({ length: totalPages }).map((_, idx) => (
            <button
              ...
              aria-label={t('templates.goToPage', { n: idx + 1 })}
            />
          ))}
        </nav>
      </motion.div>

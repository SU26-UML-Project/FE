...
export function Properties({
    ...
    onUpdateEdge: (id: string, patch: { label?: string; marker?: string; markerStart?: string; type?: string; dashed?: boolean; color?: string; flip?: boolean; markerSize?: number }) => void;
    onDelete: () => void;
    onDuplicate: () => void;
}) {
    if (node) {
        return (
            <NodeEditor
                node={node}
                allNodes={allNodes}
                onUpdateParent={onUpdateNodeParent}
    ...
    if (edge) {
        return (
            <EdgeEditor
                edge={edge}
                diagramType={diagramType}
                onChange={(patch) => onUpdateEdge(edge.id, patch)}
                onDelete={onDelete}
            />
        );
    }
...
function EdgeEditor({ edge, diagramType, onChange, onDelete }: {
    ...
    onChange: (patch: { label?: string; marker?: string; markerStart?: string; type?: string; dashed?: boolean; color?: string; flip?: boolean; markerSize?: number; multiplicitySource?: string; multiplicityTarget?: string }) => void;
    onDelete: () => void;
}) {
    const opts = getDiagram(diagramType).edges;
    const data = edge.data as { marker?: string; markerStart?: string; color?: string; markerSize?: number } | undefined;
    const marker = data?.marker ?? "";
    const markerStart = data?.markerStart ?? "";
    const color = data?.color;
    const markerSize = data?.markerSize ?? 1;
        ...
        <div className="animate-fade-in space-y-4">
            ...
            <div>
                ...
                <span className="flex h-4 w-16 shrink-0 items-center">
                  <ConnectorGlyph markerEnd={o.markerEnd} markerStart={o.markerStart} dashed={o.dashed} color={color} size={markerSize} />
                </span>
            ...
            <div>
                ...
                <MarkerPicker side="start" current={markerStart} color={color} size={markerSize} onPick={(m) => onChange({ markerStart: m })} />
            </div>
            <div>
                <label className={labelCls}>End marker</label>
                <MarkerPicker side="end" current={marker} color={color} size={markerSize} onPick={(m) => onChange({ marker: m })} />
            </div>
            <div>
                <label className={labelCls}>Arrow size</label>
                <div className="grid grid-cols-6 gap-1">
                    {MARKER_SIZES.map((s) => {
                        const active = Math.abs((markerSize || 1) - s.size) < 1e-6;
                        return (
                            <button key={s.size} title={`${Math.round(s.size * 100)}%`} onClick={() => onChange({ markerSize: s.size })}
                                    className={`flex h-8 items-center justify-center rounded-lg border text-[11px] font-semibold transition-colors ${active ? "border-zinc-900 bg-white text-zinc-900" : "border-[var(--line)] bg-white/60 text-zinc-500 hover:bg-white"}`}>
                                {`${Math.round(s.size * 100)}%`}
                            </button>
                        );
                    })}
                </div>
...
function MarkerPicker({ side, current, color, size = 1, onPick }: {
    side: "start" | "end"; current: string; color?: string; size?: number; onPick: (id: string) => void;
}) {
    return (
        <div className="grid grid-cols-6 gap-1">
            {MARKER_SHAPES.map((m: MarkerShape) => {
                ...
                return (
                    <button key={m.label} title={m.label} onClick={() => onPick(value)}
                        ...
                        {side === "start" ? <ConnectorGlyph markerStart={m.start} color={color} width={26} size={size} /> : <ConnectorGlyph markerEnd={m.end} color={color} width={26} size={size} />}
                    </button>
                );
            })}
        </div>
...
function EmptyState({ nodesLen, edgesLen, activeConnectorName }: {
    ...
    return <InspectorStatus nodesLen={nodesLen} edgesLen={edgesLen} activeConnectorName={activeConnectorName} />;
}

function InspectorStatus({ nodesLen, edgesLen, activeConnectorName }: {
    nodesLen: number; edgesLen: number; activeConnectorName: string;
}) {
    return (

...
import { getTemplateContent } from '../../../shared/lib/templates'
import type { TemplateContent } from '../../../shared/lib/templates'
...
function GraphCanvas({ previewImage, name }: { previewImage: string | null; name: string }) {
  ...
  return <img src={previewImage} alt={`Preview of ${name}`} className="h-full w-full object-contain bg-slate-50" />
}
...
export default function TemplateDetail() {
  ...
  const handleOpenSample = async () => {
    ...
    try {
      ...
      if (!projectId) throw new Error('Unable to create project')

      const sheets = await sheetService.getSheetsByProject(projectId)
      const defaultSheet = sheets.result?.[0]
      if (!defaultSheet) throw new Error('Unable to prepare diagram sheet')

      const canvas = structuredClone(template.canvasData)
      ...
      navigate(`/workspace/${projectId}/editor`)
    } catch (requestError) {
      toast.error(requestError instanceof Error ? requestError.message : 'Unable to create sample')
    } finally {
      setCreating(false)
    }
  }
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-10">
          ...
          >
            ...
            <div className="relative h-96 rounded-sm overflow-hidden border border-gray-200 bg-gray-50">
              <GraphCanvas previewImage={template.previewImage} name={template.name} />
            </div>
          </motion.div>

          {template.kind === 'sample' && <motion.div
              initial={{ opacity: 0, y: 12 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-40px' }}
          ...
          >
            <button
                ...
                disabled={creating || !template.canvasData}
                className="px-10 py-4 bg-uml-blue text-white font-bold rounded-md text-sm uppercase tracking-wider hover:bg-blue-700 transition shadow-md active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {creating ? 'Creating sample...' : 'Open in Workspace'}
            </button>
          </motion.div>}
        </div>
      </div>

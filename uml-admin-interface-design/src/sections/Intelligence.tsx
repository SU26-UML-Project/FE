...
import { Badge, Button, Card, EndpointBadge, inputCls, PageHeader } from "@/components/ui";
...
function DetectButton({ loading, onClick, label }: { loading: boolean; onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-black/5 bg-white/80 px-3 text-[12.5px] font-semibold text-ink transition hover:bg-white disabled:opacity-60"
    >
      {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ScanSearch className="h-4 w-4 text-golddk" />}
  ...
  return (
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-black/5 pt-4">
      ...
      <button
        ...
        className={cn("inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[13px] font-bold text-white transition", saved ? "bg-emerald-600" : "bg-gold hover:bg-golddk")}
      >
...
export default function Intelligence() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="AI Engine" subtitle="Cấu hình AnythingLLM · gọi trực tiếp REST API" />

      {/* connection banner */}
      <Card className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-4">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-ink text-white"><Plug className="h-6 w-6" /></span>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-[16px] font-bold text-ink">AnythingLLM Engine</h2>
            ...
            <p className="mt-0.5 font-mono text-[12px] text-gray-500">http://localhost:3001/api · v1.8.5 · Bearer ••••7f2a</p>
          </div>
        </div>
          ...
          <div className="hidden grid-cols-3 gap-5 sm:grid">
            {[{ l: "Độ trễ", v: "1,2s" }, { l: "Vector", v: "5,8M" }, { l: "Workspace", v: String(ws.length) }].map((x) => (
              <div key={x.l} className="text-center">
                <p className="text-[17px] font-bold text-ink">{x.v}</p>
                <p className="text-[11px] text-gray-400">{x.l}</p>
              </div>
            ))}
          </div>
          <button onClick={() => { setTest("loading"); setTimeout(() => setTest("ok"), 1100); }} className="inline-flex items-center gap-1.5 rounded-full border border-black/5 bg-white/80 px-4 py-2 text-[13px] font-semibold text-ink transition hover:bg-white">
        ...
        <Card className="h-fit p-2">
          <div className="flex gap-1 overflow-x-auto lg:flex-col lg:overflow-visible">
            {intelConfig.map((c) => {
              ...
              return (
                <button key={c.key} onClick={() => setCat(c.key)} className={cn("flex shrink-0 items-center gap-2.5 rounded-2xl px-2.5 py-2 text-left transition lg:w-full", active ? "bg-ink text-white shadow-soft" : "text-gray-600 hover:bg-white/60")}>
                  <Icon className={cn("h-[17px] w-[17px]", active ? "text-white" : "text-gray-400")} />
                  <span className="hidden lg:block">
                    <span className="block text-[13px] font-semibold leading-tight">{c.label}</span>
                    <span className={cn("block text-[11px]", active ? "text-gray-300" : "text-gray-400")}>{c.desc}</span>
                  </span>
                  <span className="lg:hidden text-[13px] font-semibold">{c.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <Card className="p-5">
          {cat === "connection" && (
            <div className="animate-fade-in">
              <h3 className="text-[14px] font-bold text-ink">Kết nối AnythingLLM</h3>
              <p className="text-[13px] text-gray-500">Gọi trực tiếp REST API — không nhúng giao diện bên AnythingLLM để giữ đồng bộ.</p>
          ...
          {cat === "workspaces" && (
            <div className="animate-fade-in">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-[14px] font-bold text-ink">Quản lý Workspace</h3>
                  <p className="text-[13px] text-gray-500">Không gian tri thức RAG — CRUD qua API.</p>
                </div>
                <Button onClick={() => setShowCreate((s) => !s)}><Plus className="h-4 w-4" /> Workspace</Button>
              </div>
              {showCreate && (
                <div className="mt-3 flex flex-col gap-2 rounded-2xl border border-black/5 bg-white/60 p-3 sm:flex-row sm:items-end">
                  ...
                  <Button onClick={addWs}>Tạo</Button>
                </div>
              )}
              <div className="mt-4 -mx-2 overflow-x-auto">
                <table className="w-full min-w-[600px] text-left">
                  ...
                  <tbody className="divide-y divide-black/5">
                    {ws.map((w) => (
                      <tr key={w.id} className="group transition hover:bg-white/50">
                        <td className="px-3 py-2.5">
                          <div className="flex items-center gap-2.5">
                            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface text-gray-500"><Layers className="h-4 w-4" /></span>
                            <div>
                              <p className="text-[13px] font-semibold text-ink">{w.name}</p>
                              <p className="font-mono text-[11px] text-gray-400">/{w.slug}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-3 py-2.5 text-right text-[13px] font-bold text-ink">{w.docs}</td>
                        <td className="px-3 py-2.5 text-right text-[13px] text-gray-600">{w.vectors}</td>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex justify-end opacity-0 transition group-hover:opacity-100">
                            <button className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-600" onClick={() => setWs((arr) => arr.filter((x) => x.id !== w.id))}><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {cat === "llm" && (
            <div className="animate-fade-in">
              <h3 className="text-[14px] font-bold text-ink">Cấu hình mô hình LLM</h3>
              <p className="text-[13px] text-gray-500">Ưu tiên chạy local để bảo mật dữ liệu. Bấm “Tự phát hiện” để quét mô hình từ engine cục bộ.</p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              {canDetectLlm && (
                <div className="mt-4 rounded-2xl border border-black/5 bg-white/60 p-3">
                  {detectedLlm.length > 0 ? (
                    <>
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                        <p className="text-[12.5px] text-gray-600">Đã phát hiện <b className="text-ink">{detectedLlm.length}</b> mô hình từ <span className="font-mono text-gray-500">{llmUrl}</span></p>
                      </div>
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {detectedLlm.map((m) => (
                          <button key={m} onClick={() => setModel(m)} className={cn("rounded-lg px-2 py-1 font-mono text-[11px] transition", model === m ? "bg-ink text-white" : "bg-white text-gray-600 ring-1 ring-black/5 hover:bg-surface2")}>{m}</button>
                        ))}
                      </div>
                    </>
                  ) : (
                    <div className="flex flex-wrap items-center gap-3">
                      <p className="text-[12.5px] text-gray-500">Quét mô hình cục bộ thông qua</p>
              ...
              <div className="mt-4 rounded-2xl border border-black/5 bg-white/70 p-4">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-ink">Temperature</span>
                  <span className="rounded-full bg-surface px-2 py-0.5 font-mono text-[12px] font-bold text-ink">{temp.toFixed(2)}</span>
                </div>
                <input type="range" min={0} max={1} step={0.05} value={temp} onChange={(e) => setTemp(parseFloat(e.target.value))} className="mt-3 w-full accent-[#e4a11b]" />
                <div className="mt-1 flex justify-between text-[11px] text-gray-400"><span>Tuyến tính</span><span>Sáng tạo</span></div>
              </div>
          ...
          {cat === "embeddings" && (
            <div className="animate-fade-in">
              <h3 className="text-[14px] font-bold text-ink">Mô hình Embedding</h3>
              <p className="text-[13px] text-gray-500">Đổi embedder sẽ cần re-embed toàn bộ corpus.</p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="mt-4 rounded-2xl border border-black/5 bg-white/60 p-3">
                  {detectedEmb.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-1.5">
                      <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                      <p className="mr-2 text-[12.5px] text-gray-600">Phát hiện <b className="text-ink">{detectedEmb.length}</b> mô hình nhúng:</p>
                      {detectedEmb.map((m) => (
                        <button key={m} onClick={() => setEmb(m)} className={cn("rounded-lg px-2 py-1 font-mono text-[11px] transition", emb === m ? "bg-ink text-white" : "bg-white text-gray-600 ring-1 ring-black/5 hover:bg-surface2")}>{m}</button>
                      ))}
                    </div>
                  ) : (
                    <div className="flex flex-wrap items-center gap-3">
                      <p className="text-[12.5px] text-gray-500">Quét mô hình nhúng cục bộ thông qua</p>
                      <EndpointBadge method="GET" path="/api/tags" className="bg-white" />
                    </div>
                  )}
                </div>
              )}
              <div className="mt-4 flex items-center gap-2 rounded-xl bg-amber-50 p-3">
          ...
          {cat === "vectordb" && (
            <div className="animate-fade-in">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-[14px] font-bold text-ink">Vector Database</h3>
                  <p className="text-[13px] text-gray-500">Chọn nhà cung cấp lưu trữ vector. Bấm “Thêm” để cấu hình nhà cung cấp mới.</p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {activeVdbs.map((inst) => {
                  ...
                  return (
                    <div key={p.id} className={cn("relative rounded-2xl border p-4 transition", active ? "border-ink bg-white/70 shadow-soft" : "border-white/60 hover:border-black/10")}>
                      <button onClick={() => setVdb(p.id)} className="block w-full text-left">
                        <span className="flex h-9 w-9 items-center justify-center rounded-xl text-white" style={{ backgroundColor: p.color }}><Database className="h-4 w-4" /></span>
                        <p className="mt-2 text-[13px] font-semibold text-ink">{p.name}</p>
                        <p className="text-[11px] text-gray-400">{kindLabel[p.kind]}</p>
                        {urlVal && <p className="mt-1 truncate font-mono text-[10.5px] text-gray-400">{urlVal}</p>}
                        {active && <p className="mt-1.5 text-[10.5px] font-bold text-golddk">● Đang dùng</p>}
                      </button>
                      {p.id !== "lancedb" && (
                        <button onClick={() => removeVdb(p.id)} className="absolute right-1.5 top-1.5 rounded p-1 text-gray-300 transition hover:bg-rose-50 hover:text-rose-600"><X className="h-3.5 w-3.5" /></button>
                      )}
                    </div>
                  );
                })}
                <button onClick={() => { setVdbModal(true); setVdbForm({}); }} className="flex min-h-[120px] flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-black/10 text-gray-400 transition hover:border-gold hover:text-golddk">
                  <Plus className="h-6 w-6" />
                  <span className="text-[12.5px] font-semibold">Thêm nhà cung cấp</span>
                </button>
              </div>
              <SaveBar endpoint={{ method: "PUT", path: "/api/v1/system/preferences" }} />
            </div>
            ...
            <div className="animate-fade-in">
              <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-[13px] text-gray-500">Hỗ trợ duy nhất định dạng .md — tạo, xem, sửa, xoá tài liệu.</p>
                </div>
              ...
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <label className="relative flex items-center">
                  ...
                  <input value={docSearch} onChange={(e) => setDocSearch(e.target.value)} placeholder="Tìm tài liệu…" className={cn(inputCls, "w-44 pl-9 sm:w-56")} />
                </label>
                <div className="flex flex-wrap gap-1 rounded-full border border-white/60 bg-surface p-1 shadow-soft">
                  <button onClick={() => setDocWs("all")} className={cn("rounded-full px-2.5 py-1.5 text-[12px] font-semibold transition", docWs === "all" ? "bg-white text-ink shadow-sm" : "text-gray-500 hover:text-ink")}>Tất cả</button>
                  {wsSlugs.map((s) => (
                    <button key={s} onClick={() => setDocWs(s)} className={cn("rounded-full px-2.5 py-1.5 font-mono text-[11.5px] font-semibold transition", docWs === s ? "bg-white text-ink shadow-sm" : "text-gray-500 hover:text-ink")}>{s}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                {filteredDocs.map((d) => (
                  <div key={d.id} className="group flex items-center gap-3 rounded-2xl border border-white/60 bg-white/50 p-3 transition hover:bg-white/70">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="font-mono text-[11px] text-gray-400">{d.workspace} · {d.size}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-0.5">
                      <button onClick={() => setViewDoc(d)} disabled={d.status !== "ready"} title="Xem" className="rounded-lg p-1.5 text-gray-400 transition hover:bg-white hover:text-ink disabled:opacity-30"><Eye className="h-4 w-4" /></button>
                      <button onClick={() => openEdit(d)} title="Sửa" className="rounded-lg p-1.5 text-gray-400 transition hover:bg-white hover:text-ink"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => setConfirmDel(d)} title="Xoá" className="rounded-lg p-1.5 text-gray-400 transition hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
                {filteredDocs.length === 0 && (
                  <p className="py-10 text-center text-[13px] text-gray-400">Chưa có tài liệu nào phù hợp.</p>
                )}
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
      ...
      <Card className="p-5">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-[14px] font-bold text-ink">Giám sát API theo thời gian thực</h3>
          ...
          {apiCalls.map((c, i) => (
            <div key={i} className="flex items-center gap-3 rounded-xl px-2 py-2 transition hover:bg-white/50">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <span className="w-12 text-right font-mono text-[12px] text-gray-500">{c.t}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-[210px_1fr]">
          <div className="scroll-slim max-h-[52vh] space-y-1 overflow-y-auto pr-1">
            {vectorDbCatalog.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => { setVdbPick(p.id); setVdbForm({}); }} className={cn("flex w-full items-center gap-2.5 rounded-xl border px-2.5 py-2 text-left transition", sel ? "border-ink bg-white/70 shadow-soft" : "border-black/5 hover:bg-white/60")}>
                  <span className="flex h-7 w-7 items-center justify-center rounded-lg text-white" style={{ backgroundColor: p.color }}><Database className="h-3.5 w-3.5" /></span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[12.5px] font-semibold text-ink">{p.name}</span>
                    <span className="block text-[10.5px] text-gray-400">{kindLabel[p.kind]}</span>
                  </span>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <p className="mt-3 text-[12.5px] leading-relaxed text-gray-500">{p.desc}</p>
                  {p.fields.length === 0 ? (
                    <div className="mt-4 flex items-center gap-2 rounded-xl bg-surface p-3 text-[12.5px] text-gray-600">
                      <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /> Không cần cấu hình — chạy nhúng sẵn, không yêu cầu URL hay API Key.
                    </div>
                  ) : (
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-black/5 pt-4">
                    ...
                    <Button onClick={addVdb} className={cn(added && "opacity-50")}><Plus className="h-4 w-4" /> {added ? "Đã thêm" : "Thêm nhà cung cấp"}</Button>
                  </div>
                </>
              );
            })()}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={doDelete} className="inline-flex items-center gap-1.5 rounded-full bg-rose-600 px-4 py-2 text-[13px] font-bold text-white transition hover:bg-rose-700"><Trash2 className="h-4 w-4" /> Xoá</button>
          </>
        }
      >
        <p className="text-[13px] leading-relaxed text-gray-600">
          Tài liệu <b className="text-ink">{confirmDel?.name}</b> sẽ bị gỡ khỏi workspace{" "}
          <span className="font-mono text-gray-500">{confirmDel?.workspace}</span> và xoá khỏi vector DB. Hành động này không thể hoàn tác.
        </p>
      </Modal>
    </div>
  );

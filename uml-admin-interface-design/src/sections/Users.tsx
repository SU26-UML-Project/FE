...
import { Avatar, Badge, Card, inputCls, PageHeader } from "@/components/ui";
...
function MiniStat({
  ...
  return (
    <Card className="flex items-center gap-3 p-4">
      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/70 text-gray-600">
        <Icon className="h-5 w-5" />
      </span>
      <div>
        <p className="text-[12px] text-gray-500">{label}</p>
        <p className="text-[19px] font-bold leading-tight text-ink">{value}</p>
      </div>
...
export default function Users() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Người dùng" subtitle="Quản lý tài khoản và phân quyền trên toàn hệ thống" />
      ...
      <Card className="p-5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-1.5 rounded-full border border-white/60 bg-surface p-1 shadow-soft">
            {(["all", "Admin", "Manager", "Member", "Viewer"] as const).map((r) => (
              <button
                ...
                className={cn(
                  ...
                  role === r ? "bg-white text-ink shadow-sm" : "text-gray-500 hover:text-ink"
                )}
              >
                {r === "all" ? "Tất cả" : r}
              </button>
          ...
          <div className="flex items-center gap-2">
            <label className="relative flex items-center">
              ...
              <input
                ...
                className={cn(inputCls, "w-full pl-9 sm:w-56")}
              />
            </label>
            <button className="inline-flex items-center gap-1.5 rounded-full bg-gold px-4 py-2 text-[13px] font-bold text-white shadow-soft transition hover:bg-golddk">
        ...
        <div className="-mx-2 overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-black/5">
              {filtered.map((u) => (
                <tr key={u.id} className="group transition hover:bg-white/50">
                  ...
                  <td className="px-3 py-2.5 text-right text-[13.5px] font-bold text-ink">{u.projects}</td>
                  <td className="px-3 py-2.5 text-right text-[13.5px] text-gray-600">{u.diagrams}</td>
                  <td className="px-3 py-2.5">
                    <span className={cn("text-[12.5px]", u.lastActive === "Đang hoạt động" ? "font-semibold text-emerald-600" : "text-gray-500")}>
                      {u.lastActive}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center justify-end gap-0.5 opacity-0 transition group-hover:opacity-100">
                      <button className="rounded-lg p-1.5 text-gray-400 hover:bg-white hover:text-ink"><Pencil className="h-4 w-4" /></button>
                      <button className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>
                      <button className="rounded-lg p-1.5 text-gray-400 hover:bg-white"><MoreHorizontal className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {filtered.length === 0 && (
            <p className="py-10 text-center text-[13px] text-gray-400">Không tìm thấy người dùng phù hợp.</p>
          )}
        </div>
      </Card>
    </div>

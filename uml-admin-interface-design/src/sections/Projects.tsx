...
import { Avatar, Badge, Card, PageHeader, Segmented } from "@/components/ui";
...
export default function Projects() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Dự án" subtitle="Tất cả dự án UML trên toàn hệ thống" />
      ...
      <Card className="p-5">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-[14px] font-bold text-ink">Tất cả dự án</p>
        ...
        {view === "grid" ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {projects.map((p) => (
              <div key={p.id} className="rounded-2xl border border-white/60 bg-white/70 p-4 shadow-soft hover-lift">
                <div className="flex items-start justify-between">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface text-gray-600">
                    <FolderKanban className="h-5 w-5" />
                  </span>
                  <Badge tone={projectStatus[p.status].tone}>{projectStatus[p.status].label}</Badge>
                </div>
                <h4 className="mt-3 text-[14.5px] font-bold text-ink">{p.name}</h4>
                <p className="text-[12px] text-gray-400">{p.category}</p>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="rounded-xl bg-surface p-2">
                    <p className="text-[11px] text-gray-400">Sơ đồ</p>
                    <p className="text-[15px] font-bold text-ink">{p.diagrams}</p>
                  </div>
                  <div className="rounded-xl bg-surface p-2">
                    <p className="text-[11px] text-gray-400">Thành viên</p>
                    <p className="text-[15px] font-bold text-ink">{p.members}</p>
                  </div>
                </div>
                <div className="mt-3 flex items-center justify-between border-t border-black/5 pt-3">
                  <div className="flex items-center gap-2">
                    <Avatar initials={ownerInitials(p.owner)} color={ownerColors[p.owner]} size="h-6 w-6" />
                    <span className="text-[12px] text-gray-500">{p.owner}</span>
                  </div>
                  <span className="text-[11.5px] text-gray-400">{p.updated}</span>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tr key={p.id} className="transition hover:bg-white/50">
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface text-gray-500"><FolderKanban className="h-4 w-4" /></span>
                        <div>
                          <p className="text-[13px] font-semibold text-ink">{p.name}</p>
                          <p className="text-[11.5px] text-gray-400">{p.category}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <Avatar initials={ownerInitials(p.owner)} color={ownerColors[p.owner]} size="h-6 w-6" />
                        <span className="text-[12.5px] text-gray-600">{p.owner}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2.5 text-right text-[13px] font-bold text-ink">{p.diagrams}</td>
                    <td className="px-3 py-2.5 text-right text-[13px] text-gray-600">{p.members}</td>
                    <td className="px-3 py-2.5"><Badge tone={projectStatus[p.status].tone}>{projectStatus[p.status].label}</Badge></td>
                    <td className="px-3 py-2.5 text-[12px] text-gray-400">{p.updated}</td>
                  </tr>
                ))}
              </tbody>
            </table>
